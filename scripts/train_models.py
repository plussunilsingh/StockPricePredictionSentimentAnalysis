"""Training orchestration script for baseline models (RF and LSTM).

This script is intentionally explicit and small: it loads historical data
from the mock scanner, computes technical indicators, and trains two
baseline models. It is designed to be easy to reproduce locally.
"""
...
def train():
    ...
    # If sentiment data is not available, use neutral scores (0.0)
    ...
    print(f"RF Model trained with {x_rf.shape[1] if x_rf.size else 0} features.")
    ...
    print(f"LSTM Model trained with shape {x_lstm.shape if hasattr(x_lstm, 'shape') else None}.")
