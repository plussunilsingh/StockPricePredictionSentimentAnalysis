"""Feature engineering helpers for technical indicators and sentiment merging.

This module intentionally keeps functions small and well-documented so
reviewers can easily understand each derived column and its purpose.
"""
...
class FeatureEngineer:
    ...
    def mergeSentiment(stockData: 'pd.DataFrame', newsData: 'pd.DataFrame', sentimentAnalyzer) -> 'pd.DataFrame':
        ...
        if 'Close' not in stockData.columns:
            # Defensive: if Close is missing, return stockData with neutral sentiment
